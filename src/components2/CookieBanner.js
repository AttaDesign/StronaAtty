    ...
    return (
        <div className={`my-10 mx-auto max-w-max md:max-w-screen-sm
            ...
            <div className='text-center'>
                <Link href="/politykaprywatnosci"><p className="text-white text-light">We use <span className='font-bold text-bold text-blue-500 shadow-lg'>cookies</span> on our site.</p></Link>
            </div>

            
            <div className='flex gap-2'>
                <button className='px-5 py-2 text-gray-300 rounded-md border-gray-900' onClick={() => setCookieConsent(false)}>Decline</button>
                <button className='bg-blue-900 px-5 py-2 text-white rounded-lg' onClick={() => setCookieConsent(true)}>Allow Cookies</button>
            </div>   
        </div>
    )}
